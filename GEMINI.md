...
KotlinおよびCompose Multiplatformの一般的な命名規則に従ってください。ただし、Swiftの命名規則も参考にしてください。Swiftの命名規則も考慮した場合はコメントにて記載してください。
...
- **プラットフォーム依存の回避**：可能な限り、`composeApp/src/commonMain/`ディレクトリ内でコードを記述し、プラットフォーム依存のコードは`androidMain`および`iosMain`ディレクトリ内に記述してください。
...
- **言語**：日本語で回答してください。
- **コードブロック**：成生するコードは必ずファイルパス(例：`composeApp/src/commonMain/kotlin/com/example/MyFile.kt`)を明記してください。
- **プラットフォームの優先順位**：質問に指定がない限り、Android/iOS両方で動作する`commonMain`向けのコードを優先して提示してください。
- **参照・ドキュメント**：生成するコードにおいて参照したサイトやドキュメントは必ず最後に記載してください。
